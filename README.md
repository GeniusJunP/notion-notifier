...
1. **高度な Notion データベース連携 (Property Mapping & Filtering)**
...
4. **Bi-directional(双方向)を意識した Google カレンダー統合**
   Notion の予定を一方向に Google カレンダーに同期するだけでなく、Notion の `people` プロパティ（参加者）を抽出し、Google カレンダーのゲストとして招待メールを送信するオプション (`attendees_enabled`) も備えています。

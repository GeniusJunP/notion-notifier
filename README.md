...
4. **Google カレンダーへの一方向の一括同期 (Uni-directional Sync)**
   Notion の予定リストを監視し、Google カレンダー側に自動で反映（作成・更新・削除）します。機能は一方向ですが、Notion の `people` プロパティから参加者を抽出し、Google カレンダーのゲストとして招待メールを送信する高度なオプション (`attendees_enabled`) も備えています。

...
Notionデータベース上の予定をDiscordに通知し、Googleカレンダーと同期する個人用ツール。
...
**重要**: API認証情報（Notion API Key、Discord Webhook、Google credentialsなど）はWebUIで編集禁止。`env.yaml`ファイルを直接編集して設定する。設定値は `config.yaml` で管理し、認証情報は環境変数でも上書き可能。
...
| `DISCORD_SCHEDULE_WEBHOOK`     | `discord.schedule_webhook`     |
| `DISCORD_NOTIFICATION_WEBHOOK` | `discord.notification_webhook` |
...
| Discord Webhook | 30 req/60sec per webhook  | 指数バックオフ（最大3回）                |

...
通知メッセージは **Go の `text/template`** を使用する。
...
| `{{.Type}}`            | 通知種別（upcoming/periodic/manual） |
| `{{.Events}}`          | 通知対象のイベント配列                 |
| `{{.Event}}`           | 先頭イベント（単一通知の簡易参照）     |
| `{{.MinutesBefore}}`   | 事前通知の分数（upcomingのみ）          |
...
| 技術          | 用途                                       |
| ------------- | ------------------------------------------ |
| **Svelte**    | コンポーネント指向 UI / リアクティブステート管理 |
| **Vite**      | 高速なフロントエンドビルドツール           |
| **Tailwind CSS** | ユーティリティファーストCSSフレームワーク  |
| **API連動**   | JSON REST API（`fetch`）経由による通信   |

### 静的アセット配信

Go の `embed` パッケージで、Viteによってビルドされた `web/dist` 配下（HTML, CSS, JS）を単一のバイナリに埋め込む。
...
Web UIはSvelteによるシングルページアプリケーション（SPA）として動作し、専用の JSON REST API (`/api/*`) と通信してデータを取得・更新する。
...
CREATE TABLE notification_history (
    ...
    type TEXT NOT NULL,        -- upcoming/periodic/manual
